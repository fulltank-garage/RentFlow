...
function readUpstreamSession(setCookieHeaders: string[]) {
  for (const value of setCookieHeaders) {
    const match = value.match(/(?:^|,\s*)rentflow_partner_session=([^;,\s]+)/);
...
export const GET = proxyPartnerRequest;
export const POST = proxyPartnerRequest;
export const PUT = proxyPartnerRequest;
export const PATCH = proxyPartnerRequest;
export const DELETE = proxyPartnerRequest;
